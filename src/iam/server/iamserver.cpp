...
static const Array<uint8_t> ConvertByteArrayToAOS(const std::string& arr)
{
    return {reinterpret_cast<const uint8_t*>(arr.c_str()), arr.length()};
}

static std::string ConvertByteArrayToProto(const Array<uint8_t>& arr)
{
...
static InstanceIdent ConvertToAos(const iamanager::v4::InstanceIdent& val)
{
...
static void ConvertToProto(const Array<StaticString<cSubjectIDLen>>& src, iamanager::v4::Subjects& dst)
{
    dst.clear_subjects();
...
Error IAMServer::Init(const Config& config, certhandler::CertHandlerItf& certHandler,
    identhandler::IdentHandlerItf* identHandler, permhandler::PermHandlerItf* permHandler, IAMClientItf* remoteHandler,
    cryptoutils::CertLoader& certLoader, crypto::x509::ProviderItf& cryptoProvider, bool provisioningMode)
{
    ...
    try {
        ...
        if (!provisioningMode) {
            ...
            protectedOpt = GetMTLSServerCredentials(certInfo, certLoader, cryptoProvider);
        } else {
    ...
    grpc::ServerContext* context, const google::protobuf::Empty* request, iamanager::v4::SystemInfo* response)
{
    ...
    Tie(systemID, err) = mIdentHandler->GetSystemID();
    if (!err.IsNone()) {
        LOG_DBG() << "Get system ID error: " << err;
    ...
    Tie(boardModel, err) = mIdentHandler->GetUnitModel();
    if (!err.IsNone()) {
        LOG_DBG() << "Get board model error: " << err;
    ...
    iamanager::v4::PermissionsResponse* response)
{
    ...
    if (!err.IsNone()) {
        LOG_DBG() << "Get permissions error: " << err;

        return grpc::Status(grpc::StatusCode::INTERNAL, "get permissions error");
    }
    ...
    iamanager::v4::CreateKeyResponse* response)
{
    ...
    if (subject.IsEmpty() && mIdentHandler) {
        ...
        if (!err.IsNone()) {
            LOG_ERR() << "Get system ID returned error=" << err;

            return grpc::Status(grpc::StatusCode::INTERNAL, "get system id returned error");
        }
    }
